    ...
    private (thread_id)
  {
    ...
    for(cycleI = start; cycleI < end; ++ cycleI){
      for(cycleJ = 0; cycleJ < matrixB.xDim - 3; cycleJ += 4){
        // set acc to 0
        acc_0 = __builtin_ia32_xorps(acc_0, acc_0);
        acc_1 = __builtin_ia32_xorps(acc_1, acc_1);
        acc_2 = __builtin_ia32_xorps(acc_2, acc_2);
        acc_3 = __builtin_ia32_xorps(acc_3, acc_3);
